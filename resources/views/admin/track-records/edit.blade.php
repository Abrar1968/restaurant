@extends('layouts.admin')

@section('title', 'Edit Track Record')

@section('content')
<div class="space-y-6">
    {{-- Page Header --}}
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.track-records.index') }}" class="text-gray-400 hover:text-white transition">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                </svg>
            </a>
            <h1 class="text-2xl font-bold text-white">Edit Track Record</h1>
        </div>
    </div>

    {{-- Errors --}}
    @if($errors->any())
        <div class="bg-red-500/10 border border-red-500/30 text-red-400 px-4 py-3 rounded-lg mb-6">
            <ul class="list-disc list-inside space-y-1">
                @foreach($errors->all() as $error)
                    <li>{{ $error }}</li>
                @endforeach
            </ul>
        </div>
    @endif

    {{-- Form --}}
    <form action="{{ route('admin.track-records.update', $record) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')

        <div class="bg-[#111] border border-white/10 rounded-lg p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {{-- Title --}}
                <div>
                    <label for="title" class="block text-sm font-medium text-gray-300 mb-1.5">Title</label>
                    <input type="text" name="title" id="title" value="{{ old('title', $record->title) }}" required
                           class="bg-[#1A1A1A] border border-white/10 rounded px-4 py-2.5 text-white focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] outline-none w-full"
                           placeholder="e.g., Annual Corporate Dinner">
                </div>

                {{-- Client Name --}}
                <div>
                    <label for="client_name" class="block text-sm font-medium text-gray-300 mb-1.5">Client Name</label>
                    <input type="text" name="client_name" id="client_name" value="{{ old('client_name', $record->client_name) }}" required
                           class="bg-[#1A1A1A] border border-white/10 rounded px-4 py-2.5 text-white focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] outline-none w-full"
                           placeholder="e.g., Petronas">
                </div>

                {{-- Year --}}
                <div>
                    <label for="year" class="block text-sm font-medium text-gray-300 mb-1.5">Year</label>
                    <input type="number" name="year" id="year" value="{{ old('year', $record->year) }}" min="2000" max="2030" required
                           class="bg-[#1A1A1A] border border-white/10 rounded px-4 py-2.5 text-white focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] outline-none w-full">
                </div>

                {{-- Event Type --}}
                <div>
                    <label for="event_type" class="block text-sm font-medium text-gray-300 mb-1.5">Event Type</label>
                    <input type="text" name="event_type" id="event_type" value="{{ old('event_type', $record->event_type) }}" required
                           class="bg-[#1A1A1A] border border-white/10 rounded px-4 py-2.5 text-white focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] outline-none w-full"
                           placeholder="e.g., Conference, Corporate Event, Wedding">
                </div>

                {{-- Sort Order --}}
                <div>
                    <label for="sort_order" class="block text-sm font-medium text-gray-300 mb-1.5">Sort Order</label>
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $record->sort_order) }}" min="0"
                           class="bg-[#1A1A1A] border border-white/10 rounded px-4 py-2.5 text-white focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] outline-none w-full">
                </div>

                {{-- Active --}}
                <div class="flex items-center pt-7">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $record->is_active) ? 'checked' : '' }}
                               class="rounded border-white/20 bg-[#1A1A1A] text-[#C9A84C] focus:ring-[#C9A84C]">
                        <span class="text-sm font-medium text-gray-300">Active</span>
                    </label>
                </div>
            </div>

            {{-- Description --}}
            <div>
                <label for="description" class="block text-sm font-medium text-gray-300 mb-1.5">Description</label>
                <textarea name="description" id="description" rows="4"
                          class="bg-[#1A1A1A] border border-white/10 rounded px-4 py-2.5 text-white focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] outline-none w-full"
                          placeholder="Brief description of the event...">{{ old('description', $record->description) }}</textarea>
            </div>

            {{-- Image Upload --}}
            <div x-data="{ preview: null }">
                <label for="image" class="block text-sm font-medium text-gray-300 mb-1.5">Image</label>

                @if($record->image)
                    <div class="mb-3">
                        <p class="text-xs text-gray-400 mb-2">Current image:</p>
                        <img src="{{ Storage::url($record->image) }}" class="w-48 h-32 object-cover rounded border border-white/10" alt="{{ $record->title }}">
                    </div>
                @endif

                <input type="file" name="image" id="image" accept="image/*"
                       @change="if ($event.target.files[0]) { const reader = new FileReader(); reader.onload = (e) => preview = e.target.result; reader.readAsDataURL($event.target.files[0]); }"
                       class="bg-[#1A1A1A] border border-white/10 rounded px-4 py-2.5 text-white focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] outline-none w-full file:mr-4 file:py-1 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-[#C9A84C]/20 file:text-[#C9A84C] hover:file:bg-[#C9A84C]/30">
                <template x-if="preview">
                    <img :src="preview" class="mt-3 w-48 h-32 object-cover rounded border border-white/10" alt="New preview">
                </template>
            </div>
        </div>

        {{-- Submit --}}
        <div class="flex items-center justify-end gap-3 mt-6">
            <a href="{{ route('admin.track-records.index') }}"
               class="border border-[#C9A84C] text-[#C9A84C] hover:bg-[#C9A84C] hover:text-black px-4 py-2 rounded transition">
                Cancel
            </a>
            <button type="submit"
                    class="bg-[#C9A84C] hover:bg-[#b8993f] text-black font-semibold px-4 py-2 rounded transition">
                Update Record
            </button>
        </div>
    </form>
</div>
@endsection
