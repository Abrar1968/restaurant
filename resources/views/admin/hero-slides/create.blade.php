...
<div class="space-y-8">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white">Add Hero Slide</h1>
            <p class="text-gray-400 mt-1">Create a new hero banner slide</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.hero-slides.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-[#111] border border-white/10 rounded-lg p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="page" id="page"
                        ...
                        <option value="home" @selected(old('page') === 'home')>Home</option>
                        <option value="about" @selected(old('page') === 'about')>About</option>
                        <option value="menu" @selected(old('page') === 'menu')>Menu</option>
                        <option value="gallery" @selected(old('page') === 'gallery')>Gallery</option>
                        <option value="track-record" @selected(old('page') === 'track-record')>Track Record</option>
                        <option value="certifications" @selected(old('page') === 'certifications')>Certifications</option>
                        <option value="contact" @selected(old('page') === 'contact')>Contact</option>
                        <option value="clients" @selected(old('page') === 'clients')>Clients</option>
                    </select>
                    @error('page')
            ...
            {{-- Image Upload --}}
            <div>
                <label for="image" class="block text-sm font-medium text-gray-300 mb-1.5">Hero Image</label>
