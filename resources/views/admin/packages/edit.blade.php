...
<div class="space-y-8" x-data="{ name: '{{ old('name', $package->name) }}', slug: '{{ old('slug', $package->slug) }}', imagePreview: null }" x-effect="slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')">
    {{-- Page Header --}}
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-gray-400 mt-1">Update details for <strong class="text-white">{{ $package->name }}</strong></p>
        </div>
    </div>
        ...
        <div class="bg-[#111] border border-white/10 rounded-lg p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="bg-[#1A1A1A] border border-white/10 rounded px-4 py-2.5 text-white focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] outline-none w-full"
                          placeholder="Describe this catering package">{{ old('description', $package->description) }}</textarea>
                @error('description')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            {{-- Current Image --}}
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-1.5">Current Cover Image</label>
            ...
            {{-- Replace Image --}}
            <div>
                <label for="cover_image" class="block text-sm font-medium text-gray-300 mb-1.5">Replace Cover Image</label>
