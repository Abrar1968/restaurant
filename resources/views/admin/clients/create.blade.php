...
<div class="space-y-8">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white">Add Client</h1>
            <p class="text-gray-400 mt-1">Add a new client to the showcase</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.clients.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-[#111] border border-white/10 rounded-lg p-6 space-y-6">
            ...
            <div x-data="{ preview: null }">
                <label for="logo" class="block text-sm font-medium text-gray-300 mb-1.5">Logo</label>
                ...
                <p class="text-gray-500 text-xs mt-1">Recommended: PNG with transparent background. Max 2MB.</p>
                @error('logo')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
