...
<div class="space-y-8" x-data="{ name: '{{ old('name') }}', slug: '{{ old('slug') }}', imagePreview: null }" x-effect="slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white">Create Menu Category</h1>
            <p class="text-gray-400 mt-1">Add a new menu category to your collection</p>
        </div>
    </div>
        ...
        <div class="bg-[#111] border border-white/10 rounded-lg p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="bg-[#1A1A1A] border border-white/10 rounded px-4 py-2.5 text-white focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] outline-none w-full"
                          placeholder="Describe this menu category">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            {{-- Image Upload --}}
            <div>
                <label for="image" class="block text-sm font-medium text-gray-300 mb-1.5">Cover Image</label>
