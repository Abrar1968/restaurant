...
<div class="space-y-8" x-data="{ imagePreview: null }">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white">Edit Menu Item</h1>
            <p class="text-gray-400 mt-1">Update details for <strong class="text-white">{{ $item->name }}</strong></p>
        </div>
    </div>
        ...
        <div class="bg-[#111] border border-white/10 rounded-lg p-6 space-y-6">
                ...
                <div>
                    ...
                    <select name="menu_id" id="menu_id"
                        ...
                        @foreach($menus as $menu)
                            <option value="{{ $menu->id }}" @selected(old('menu_id', $item->menu_id) == $menu->id)>{{ $menu->name }}</option>
                        @endforeach
                    </select>
                    @error('menu_id')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="bg-[#1A1A1A] border border-white/10 rounded px-4 py-2.5 text-white focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] outline-none w-full"
                          placeholder="Brief description of this menu item">{{ old('description', $item->description) }}</textarea>
                @error('description')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="tags" id="tags"
                           value="{{ old('tags', is_array($item->tags) ? implode(', ', $item->tags) : $item->tags) }}"
                           class="bg-[#1A1A1A] border border-white/10 rounded px-4 py-2.5 text-white focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] outline-none w-full"
                ...
                @if($item->image_path)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $item->image_path) }}" alt="{{ $item->name }}"
                             class="max-w-xs h-40 object-cover rounded-lg border border-white/10">
                    </div>
                @else
                    <div class="mb-3 w-full max-w-xs h-40 bg-[#1A1A1A] rounded-lg border border-white/10 flex items-center justify-center">
            ...
            <div>
                ...
                <p class="text-gray-500 text-xs mt-1">Leave empty to keep the current image. Recommended: 800x600px, JPEG or PNG. Max 5MB.</p>
                @error('image')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
                {{-- New Image Preview --}}
